...
def create_stock_transactions(stock_transactions_data: List[StockTransactionRequest], db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to process stock transactions")
...
def read_stock_transaction(stock_transaction_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to fetch stock transaction")
...
def list_stock_transactions(
    ...
    sort_by: str = Query("transaction_date", description="Field to sort by", regex=r"^(id|ticker_symbol|transaction_type|quantity|price_per_unit|transaction_date)$"),
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to fetch stock transactions")
...
def update_stock_transaction(stock_transaction_id: int, stock_transaction_data: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update stock transaction")
...
def delete_stock_transaction(stock_transaction_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete stock transaction")
